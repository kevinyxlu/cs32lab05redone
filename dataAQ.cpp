...
void dataAQ::createComboPoliceData(std::vector<shared_ptr<psData> >& theData) {
    ...
    for(auto incident : theData){
        ...
        allComboPoliceData[stateOfIncident]->addPSData(incident);
    }
}

bool comparePovRates(shared_ptr<demogCombo> i, shared_ptr<demogCombo> j){
    return (i->getPovertyPercent() > j->getPovertyPercent());
}
...
void dataAQ::reportTopTenStatesPS() {
    ...
    for(int i = 0 ; i < 10 ; i++){
        ...
        cout << "\nPercent below poverty: " << allComboDemogData[stateVec[i]->getState()]->getPovertyPercent();
        cout << endl;
    }
